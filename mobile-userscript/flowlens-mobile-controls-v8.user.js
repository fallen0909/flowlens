// ==UserScript==
// @name         瀑光 FlowLens 手机控制 V8
// @namespace    local.flowlens.mobile.controls.v8
// @version      1.4.8
// @description  固定左上角信息、图片贴顶、右侧工具栏、筛选图标按钮、大图自动切换和视频结束下一条。
// @match        *://*/*
...
(() => {
  if (window.__flowLensMobileControlsV8) return;
  window.__flowLensMobileControlsV8 = true;

  const VIDEO_RE = /\.(?:mp4|webm|mov|m4v)(?:[?#]|$)/i;
  let autoPlaying = false;
  let autoTimer = 0;
  let applyTimer = 0;
  let lastOpenedTileUrl = "";
  let lastMediaToken = "";
  let lastZoom = "";
  let centerTimer = 0;
  let tapGuardUntil = 0;
  ...
  const css = `
    ...
    html.xiv-active #xiv-root #xiv-topbar .xiv-actions > * { flex: 0 0 auto !important; }
    html.xiv-active #xiv-root #xiv-topbar [data-xiv="top"] { display: none !important; }
    html.xiv-active #xiv-root #xiv-filter-cycle-v8 {
    ...
    html.xiv-active #xiv-root #xiv-filter-cycle-v8 svg {
    ...
    #xiv-auto-v8::before { content: "▶"; margin-left: 2px; }
    #xiv-auto-v8[data-playing="true"] { background: rgba(180,210,255,.82) !important; border-color: rgba(116,173,255,.75) !important; }
    #xiv-auto-v8[data-playing="true"]::before { content: "Ⅱ"; margin-left: 0; letter-spacing: -2px; }
    @media (max-width: 380px) {
      ...
      #xiv-auto-v8 { right: 110px !important; width: 40px !important; height: 40px !important; min-width: 40px !important; }
    }
  `;

  function injectStyle() {
    if (document.getElementById("xiv-mobile-controls-v8-style")) return;
    const style = document.createElement("style");
    style.id = "xiv-mobile-controls-v8-style";
  ...
  function cycleFilter() {
    ...
    const i = Math.max(0, order.indexOf(filterValue()));
    setFilter(order[(i + 1) % order.length]);
  }
  function syncFilterButton() {
    const box = actions();
    if (!box) return;
    let btn = document.getElementById("xiv-filter-cycle-v8");
    if (!btn) {
      btn = document.createElement("button");
      btn.id = "xiv-filter-cycle-v8";
      btn.className = "xiv-btn";
      btn.type = "button";
      const h = (e) => {
        e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation?.(); cycleFilter();
      };
      ["pointerdown", "touchstart", "mousedown", "click", "touchend"].forEach((t) => btn.addEventListener(t, h, true));
      box.insertBefore(btn, box.firstChild);
    }
    const v = filterValue();
    btn.innerHTML = ICONS[v] || ICONS.all;
    btn.title = v === "image" ? "当前：只看图片" : v === "video" ? "当前：只看视频" : "当前：全部";
  }

  function isVideoTile(tile) {
    const url = tile?.dataset?.url || "";
    return VIDEO_RE.test(url) || !!tile?.querySelector?.("video, .xiv-video-mark");
  }
  function tiles() {
    const all = [...document.querySelectorAll("#xiv-grid .xiv-tile[data-url]")].sort((a, b) => Number(a.dataset.index || 0) - Number(b.dataset.index || 0));
    const f = filterValue();
    return all.filter((t) => {
      if (t.hidden) return false;
      if (f === "image" && isVideoTile(t)) return false;
      if (f === "video" && !isVideoTile(t)) return false;
      return true;
    });
  }
  function cleanUrl(url) { return String(url || "").split("#")[0].split("?")[0].replace(/-\d+x\d+(?=\.[^.]+$)/, ""); }
  function currentMediaUrl() {
    const m = lightbox()?.querySelector("img, video, iframe[data-media-url], .xiv-video-frame[data-media-url]");
    return m?.dataset?.mediaUrl || m?.dataset?.sourceUrl || m?.currentSrc || m?.src || "";
  }
  function currentIndex(list) {
    if (lastOpenedTileUrl) {
      const i = list.findIndex((t) => t.dataset.url === lastOpenedTileUrl);
      if (i >= 0) return i;
    }
    const media = cleanUrl(currentMediaUrl());
    const i = list.findIndex((t) => cleanUrl(t.dataset.url) === media || media.endsWith(cleanUrl(t.dataset.url).split("/").pop() || "__"));
    return i >= 0 ? i : 0;
  }
  function openTile(tile) {
    if (!tile) return false;
    lastOpenedTileUrl = tile.dataset.url || "";
    const r = tile.getBoundingClientRect?.() || { left: 20, top: 20 };
    const x = Math.max(1, r.left + Math.min(10, Math.max(1, r.width / 2)));
    const y = Math.max(1, r.top + Math.min(10, Math.max(1, r.height / 2)));
    tile.dataset.downX = String(x);
    tile.dataset.downY = String(y);
    try { tile.dispatchEvent(new PointerEvent("pointerdown", { bubbles: true, cancelable: true, pointerId: 9, clientX: x, clientY: y, button: 0 })); } catch {}
    tile.dispatchEvent(new MouseEvent("click", { bubbles: true, cancelable: true, view: window, clientX: x, clientY: y, button: 0 }));
    return true;
  }
  function nextTile(delta = 1) {
    const list = tiles();
    if (!list.length) return;
    const i = currentIndex(list);
    openTile(list[(i + delta + list.length) % list.length]);
  }
  ...
  function next() {
    const before = currentMediaUrl();
    nextTile(1);
    setTimeout(() => { if (lightbox() && currentMediaUrl() === before) fallbackNext(); }, 320);
  }
  function stopAuto() { autoPlaying = false; clearInterval(autoTimer); autoTimer = 0; syncAutoButton(); }
  function startAuto() { autoPlaying = true; clearInterval(autoTimer); syncAutoButton(); setTimeout(next, 120); autoTimer = setInterval(next, 1400); }
  function toggleAuto() { autoPlaying ? stopAuto() : startAuto(); }
  function syncAutoButton() {
    document.querySelectorAll(".xiv-lightbox-auto-next, [id^='xiv-mobile-auto'], #xiv-auto-v8").forEach((n) => { if (n.id !== "xiv-auto-v8") n.remove(); });
    const rt = root(); const lb = lightbox(); let btn = document.getElementById("xiv-auto-v8");
    if (!rt || !lb) { btn?.remove(); return; }
    if (!btn) {
      btn = document.createElement("button");
      btn.id = "xiv-auto-v8"; btn.type = "button"; btn.title = "自动切换大图";
      const h = (e) => { e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation?.(); const now = Date.now(); if (now < tapGuardUntil) return; tapGuardUntil = now + 360; toggleAuto(); };
      ["pointerdown", "touchstart", "mousedown", "click", "touchend"].forEach((t) => btn.addEventListener(t, h, true));
      rt.appendChild(btn);
    }
  ...
  function bindVideoEnd() {
    const lb = lightbox(); if (!lb) return;
    lb.querySelectorAll("video").forEach((v) => { if (v.dataset.flV8End === "true") return; v.dataset.flV8End = "true"; v.addEventListener("ended", () => { if (autoPlaying) setTimeout(next, 160); }); });
  }
  window.addEventListener("message", (e) => { const m = e.data || {}; if (autoPlaying && m.type === "XIV_VIDEO_TIME" && m.eventName === "ended") setTimeout(next, 160); });
  ...
  function centerOnce(force = false) {
    ...
    if (!force && lb.dataset.flV8Centered === token) return;
    lb.dataset.flV8Centered = token;
  ...
  function apply() {
    ...
    const lb = lightbox(); const media = currentMediaUrl(); const zoom = lb?.dataset.zoom || "";
    if (lb && (media !== lastMediaToken || zoom !== lastZoom)) { lastMediaToken = media; lastZoom = zoom; delete lb.dataset.flUserMovedActual; if (zoom === "actual") centerOnce(true); }
    if (!lb && autoPlaying) stopAuto();
  }
  function scheduleApply() { clearTimeout(applyTimer); applyTimer = setTimeout(apply, 80); }
  document.addEventListener("click", (e) => { const t = e.target?.closest?.("#xiv-grid .xiv-tile[data-url]"); if (t) lastOpenedTileUrl = t.dataset.url || lastOpenedTileUrl; }, true);
  ...
  new MutationObserver(scheduleApply).observe(document.documentElement, { childList: true, subtree: true, attributes: true, attributeFilter: ["data-active", "data-zoom", "src", "class", "value", "hidden"] });
