...
- 取消图片 10 秒超时修复/剔除规则，避免慢加载的正常图片被误过滤。
- 切换或关闭 CloudDrive2 视频时会暂停并卸载旧视频源，避免后台继续播放出声。
- CloudDrive2 大图视频禁用自动 fallback 重载，减少播放中卡顿。
- 只有当前图片/视频确实能进入 1:1 时才显示放大镜提示。
- 大图异步高清替换增加 token 防护，避免切换后旧媒体回写。
