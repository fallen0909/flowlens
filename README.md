...
> 把网页中的图片和视频，一键整理成沉浸式全屏瀑布流。当前版本：**v1.7.33**。
...
- 修复大图模式右上角自动播放按钮点击后不响应、误触发退出大图模式的问题，播放按钮事件会优先被拦截并独立切换自动播放状态。
- 大图进入 1:1 模式后，鼠标滚轮/触控板滚动改为缩放当前图片或视频，不再直接切换上一张、下一张；滚轮向上放大、向下缩小，并尽量保持当前视野中心。
- 加强 Pornpics `/zh/shorts/` 进入组图后的队列识别：从当前页、进入前页面和会话缓存中补齐组图队列，上一组/下一组按钮及逗号、句号快捷键可正常切换。
