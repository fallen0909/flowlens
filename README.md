...
> 把网页中的图片和视频，一键整理成沉浸式全屏瀑布流。当前版本：**v2.0.4**。
...
- 设置面板改为“显示与浏览、磁力与播放、页面收藏、高级设置”四个可折叠模块，统一字号、间距、开关和输入控件。
- 修复大图自动播放控制器冲突，以及视频结束后反复播放当前视频、不能切换下一项的问题。
- 磁力播放新增 CloudDrive2 流媒体和本地挂载文件两种方式；已有文件会立即播放，不再显示“播放等待”设置。
- PornPics 后续组按画廊 ID 和当前语言去重，只收集带图片的画廊卡片，并过滤站点标识后显示真实预览图。

## 2.0.3 更新重点

- 磁力/ED2K 的转存与播放改为油猴脚本直连 CloudDrive2，不再依赖 115 Magnet Play 插件。
- 设置中新增 CloudDrive2 地址、115 目录、API Token 和连接测试；Token 仅保存到油猴/扩展私有存储。
- 播放会自动等待 115 离线任务完成、定位视频并打开 CloudDrive2 流媒体地址。

## 2.0.2 更新重点

- 页面收藏与收藏列表移入设置，减少顶部工具栏占用。
- 大图模式移除链接抓取按钮，并修复缩放、拖动后按钮和箭头随内容漂移。
- 磁力/ED2K 面板支持 `M` 快捷键，并可批量或逐条保存到 CD2/115、逐条在浏览器播放。
...
1. 打开 CloudDrive2 网页端的“API Tokens”，创建一个允许浏览文件、搜索、创建文件夹、添加/查看离线任务和读取文件的 Token。
2. 在瀑光设置的“CloudDrive2 直连”中填写服务地址、115 转存目录与 Token，点击“测试连接”。
3. 在任意网页按 `M` 抓取 magnet/ED2K，可逐条播放、转存，或一键全部转存。
