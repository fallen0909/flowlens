...
- 大图切换速度改为按当前页面独立保存，不再跨网页继承旧速度。
- 修复 1:1 原尺寸查看时手动左右切换失效，同时继续暂停自动切换。
- 手机端 1:1 按设备像素比折算显示尺寸，优化 OPPO Find N5 这类高 DPR 屏幕。
- 手机端横向滑动先退出大屏，再退出图片流，并压缩设置面板尺寸。
- 修复 x.810114 / Twitter 视频缩略图 blob 兜底后不截图的问题。
