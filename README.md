...
- 手机端视频大图支持 1:1：点视频也会触发缩放，视频尺寸未就绪时会等 metadata 后自动进入 1:1。
- CloudDrive2 视频目录改为轻量占位卡片，避免网格里同时拉取大量 115 视频首帧造成打开和滚动卡顿。
- 图片卡片加载超时会自动修复或剔除，减少黑色空卡片留在图片流里。
- x.810114 入口拖动会强制接管 pointer 事件，减少被页面脚本抢走导致无法拖动。
- Pornpics 画廊只采集主图列表，排除 Related、评论区、推荐图和推荐视频。
