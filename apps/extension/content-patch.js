...
(() => {
  ...
  const KEEP_ACTIONS = new Set(["download", "auto", "full", "prev-set", "next-set", "queue-list", "top", "settings", "close"]);
  ...
  const css = `
    ...
    #xiv-topbar [data-xiv]:not([data-xiv="download"]):not([data-xiv="auto"]):not([data-xiv="full"]):not([data-xiv="prev-set"]):not([data-xiv="next-set"]):not([data-xiv="queue-list"]):not([data-xiv="top"]):not([data-xiv="settings"]):not([data-xiv="close"]), #xiv-topbar .xiv-select[data-xiv="filter"] { display: none !important; }
  ...
  function closePanelsOnOutside(event) { const settingsPanel = document.querySelector('#xiv-root [data-panel="settings"]'); const diagnosticsPanel = document.querySelector('#xiv-root [data-panel="diagnostics"]'); const queuePanel = document.querySelector('#xiv-root [data-panel="queue"]'); if ((!settingsPanel || settingsPanel.dataset.open !== "true") && (!diagnosticsPanel || diagnosticsPanel.dataset.open !== "true") && (!queuePanel || queuePanel.dataset.open !== "true")) return; if (event.target && event.target.closest && event.target.closest('[data-panel="settings"], [data-panel="diagnostics"], [data-panel="queue"], [data-xiv="settings"], [data-xiv="diag"], [data-xiv="queue-list"]')) return; if (settingsPanel) settingsPanel.dataset.open = "false"; if (diagnosticsPanel) diagnosticsPanel.dataset.open = "false"; if (queuePanel) queuePanel.dataset.open = "false"; }
