...
// @version      1.6.7
// @description  性能修复版：降低大量图片/视频卡顿，收藏同步低频防限流。
// @match        *://*/*
...
// @require      https://raw.githubusercontent.com/fallen0909/flowlens/master/src/patches/page-bookmarks.js?fl=1.6.7
...
(() => {
  if (window.__flowLensRelease167Patch) return;
  window.__flowLensRelease167Patch = true;
  const VERSION = "1.6.7";
  ...
  function setVersion() {
    ...
    window.__FlowLensVersion = {
      ...
      version: VERSION,
      channel: "stable",
      features: Array.from(new Set([...(prev.features || []), "page-bookmarks-auto-sync-throttled", "large-gallery-performance-mode"]))
    };
  ...
  function patchControlApi() {
    ...
    if (!api || typeof api.loadSavedPage !== "function" || api.__flowLensBookmarkOpen167) return;
    ...
    api.__flowLensBookmarkOpen167 = true;
  }
  ...
  function injectPerformanceStyle() {
    if (document.getElementById("flowlens-performance-167")) return;
    const style = document.createElement("style");
    style.id = "flowlens-performance-167";
    style.textContent = `
  ...
  function nearViewport(el, margin = 900) {
  ...
  function optimizeHeavyMedia() {
    ...
    root.dataset.perfMode = mediaCount > 180 ? "true" : "false";
    imgs.forEach((img, index) => {
      try {
        img.loading = index < 16 ? "eager" : "lazy";
        img.decoding = "async";
        if (index > 16) img.fetchPriority = "low";
      } catch {}
    });
  ...
  function startPerformancePatch() {
    ...
    const schedule = () => {
      clearTimeout(timer);
      timer = setTimeout(optimizeHeavyMedia, 300);
    };
    schedule();
    window.addEventListener("scroll", schedule, { passive: true });
    window.addEventListener("resize", schedule, { passive: true });
    new MutationObserver(schedule).observe(document.documentElement, { childList: true, subtree: true });
    setInterval(optimizeHeavyMedia, 2500);
  }
