...
(() => {
  ...
  function openTargetPageInFlow(url) {
    const target = normalizeUrl(url); if (!x810114Slug(target)) return false;
    try { sessionStorage.setItem(AUTO_KEY, target); sessionStorage.setItem(BOOKMARK_AUTO_KEY, target); } catch {}
    status("正在打开收藏页面");
    if (!sameUrl(target, location.href)) location.assign(target); else setTimeout(() => document.getElementById("xiv-launch")?.click?.(), 250);
    return true;
  }
