// ==UserScript==
// @name         瀑光 FlowLens 手机控制 V6
// @namespace    local.flowlens.mobile.action.v6
// @version      1.4.6
// @description  修复筛选切换按钮、给按钮加图标、保留爱心原位、修复大图自动下一张和视频结束自动下一条。
// @match        *://*/*
...
(() => {
  if (window.__flowLensMobileActionV6) return;
  window.__flowLensMobileActionV6 = true;
  ...
  const css = `
    html.xiv-active #xiv-root #xiv-topbar { pointer-events: none !important; }
    ...
    #xiv-mobile-auto-v6::before { content: "▶"; margin-left: 2px; }
    #xiv-mobile-auto-v6[data-playing="true"] { background: rgba(180,210,255,.82) !important; border-color: rgba(116,173,255,.75) !important; }
    #xiv-mobile-auto-v6[data-playing="true"]::before { content: "Ⅱ"; margin-left: 0; letter-spacing: -2px; }
    @media (max-width: 380px) {
      ...
      #xiv-mobile-auto-v6 { right: 110px !important; width: 40px !important; height: 40px !important; min-width: 40px !important; }
    }
  `;

  function injectStyle() {
    if (document.getElementById("xiv-mobile-action-v6-style")) return;
    const style = document.createElement("style");
    style.id = "xiv-mobile-action-v6-style";
  ...
  function filterLabel(value = filterValue()) { return value === "image" ? "图片" : value === "video" ? "视频" : "全部"; }
  ...
  function syncFilterButton() {
    const oldTop = root()?.querySelector('[data-xiv="top"]');
    if (oldTop && oldTop.dataset.xiv !== "filter-cycle") {
      const clone = oldTop.cloneNode(true);
      clone.dataset.xiv = "filter-cycle";
      oldTop.replaceWith(clone);
    }
    const btn = root()?.querySelector('[data-xiv="filter-cycle"]');
    if (!btn) return;
    const value = filterValue();
    btn.innerHTML = ICONS[value] || ICONS.all;
    btn.title = `切换筛选：当前${filterLabel(value)}`;
    btn.setAttribute("aria-label", btn.title);
    if (btn.dataset.flCycleBound !== "true") {
      btn.dataset.flCycleBound = "true";
      const handler = (event) => {
  ...
  function startAuto() {
    ...
    autoTimer = setInterval(nativeNext, 1400);
  }
  function toggleAuto() { autoPlaying ? stopAuto() : startAuto(); }

  function syncAutoButton() {
    document.querySelectorAll(".xiv-lightbox-auto-next, #xiv-mobile-auto-next, #xiv-mobile-auto-next-final, #xiv-mobile-auto-next-v2, #xiv-mobile-auto-v3, #xiv-mobile-auto-v4, #xiv-mobile-auto-v5").forEach((node) => node.remove());
    const rt = root();
    const lb = lightbox();
    let btn = document.getElementById("xiv-mobile-auto-v6");
    if (!rt || !lb) { btn?.remove(); return; }
    if (!btn) {
      btn = document.createElement("button");
      btn.id = "xiv-mobile-auto-v6";
      btn.type = "button";
      btn.title = "自动切换大图";
      const handler = (event) => {
        event.preventDefault();
  ...
  function ensureVideoHeart() {
    ...
    lb.insertBefore(button, close || lb.firstChild);
  }
  ...
  function bindVideoEnded() {
    ...
    lb.querySelectorAll("video").forEach((video) => {
      if (video.dataset.flActionV6End === "true") return;
      video.dataset.flActionV6End = "true";
      video.addEventListener("ended", () => { if (autoPlaying) setTimeout(nativeNext, 160); });
    });
  }
  window.addEventListener("message", (event) => {
    const msg = event.data || {};
    if (autoPlaying && msg.type === "XIV_VIDEO_TIME" && msg.eventName === "ended") setTimeout(nativeNext, 160);
  });
  ...
  function currentMediaUrl() {
    const node = lightbox()?.querySelector("img, video, iframe[data-media-url], .xiv-video-frame[data-media-url]");
  ...
  function centerOnce(force = false) {
    ...
    if (!force && lb.dataset.flActionV6Centered === token) return;
    lb.dataset.flActionV6Centered = token;
    const run = () => {
