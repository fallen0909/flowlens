(() => {
  ...
  function looksLikeMainContent(node) {
    ...
    return !RELATED_TEXT_RE.test(text);
  }

  function mark(node, reason = "recommendation") {
  ...
  function removeOrMark(node, doc, reason) {
    if (!node || skipNode(node) || isBodyLevel(node)) return;
    if (doc !== document) node.remove();
    else mark(node, reason);
  }
  ...
  function bestSectionForHeading(heading) {
    ...
    for (let depth = 0; current && depth < 6; depth += 1) {
      ...
      if (count >= 2 && count <= 80 && (RELATED_TEXT_RE.test(text) || RELATED_MARKER_RE.test(marker)) && !looksLikeMainContent(current)) {
        best = current;
      }
  ...
  function markFollowingFromHeading(heading, doc) {
    const section = bestSectionForHeading(heading);
    if (section) {
      removeOrMark(section, doc, "heading-section");
      return;
    }
    ...
    for (let i = 0; current && i < 90; i += 1) {
      ...
      const marker = markerOf(current);
      const count = mediaCount(current);
      const hit = current === topChildUnder(heading, parent) || RELATED_TEXT_RE.test(text) || RELATED_MARKER_RE.test(marker) || count > 0;
      if (!hit) break;
      const next = current.nextElementSibling;
      removeOrMark(current, doc, current === heading ? "heading" : "heading-following");
      if (count > 0) seenMedia += count;
  ...
  function pruneDocument(doc = document) {
    ...
    try {
      doc.querySelectorAll(HEADING_SELECTOR).forEach((node) => {
        ...
        markFollowingFromHeading(node, doc);
      });

      doc.querySelectorAll("[id],[class]").forEach((node) => {
        if (skipNode(node) || isBodyLevel(node)) return;
        const marker = markerOf(node);
        if (!RELATED_MARKER_RE.test(marker)) return;
        if (looksLikeMainContent(node)) return;
        const count = mediaCount(node);
        if (count >= 2 && count <= 80) removeOrMark(node, doc, "related-marker");
      });
    } catch {
      // Keep the host page usable even on unusual DOMs.
    }
  }
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", () => pruneDocument(document), { once: true });
  } else {
