...
(() => {
  ...
  const css = `
    ...
    #xiv-topbar [data-xiv]:not([data-xiv="download"]):not([data-xiv="auto"]):not([data-xiv="top"]):not([data-xiv="settings"]):not([data-xiv="close"]), #xiv-topbar .xiv-select[data-xiv="filter"] { display: none !important; }
    ...
    @media (max-width: 820px) { #xiv-topbar { justify-content: flex-end !important; gap: 6px !important; padding-right: max(8px, env(safe-area-inset-right, 0px)) !important; padding-left: max(8px, env(safe-area-inset-left, 0px)) !important; } #xiv-topbar .xiv-pill { display: none !important; } #xiv-topbar .xiv-btn { min-width: 42px !important; width: 42px !important; height: 42px !important; } #xiv-stage { padding-top: calc(58px + env(safe-area-inset-top, 0px)) !important; } #xiv-lightbox img, #xiv-lightbox video { max-width: 100vw !important; max-height: 100dvh !important; } }
  `;
  ...
  function ensureToolbarCompact() { document.querySelectorAll("#xiv-topbar [data-xiv]").forEach((el) => { const show = KEEP_ACTIONS.has(el.dataset.xiv); el.hidden = !show; if (!show) el.style.display = "none"; }); const filter = document.querySelector('#xiv-topbar .xiv-select[data-xiv="filter"]'); if (filter) { filter.hidden = true; filter.style.display = "none"; } }
